...
def create_db():
    ...
    try:
        ...
        logger.error(f'''DATABASE QUERY FAILED:
                     {queries['create_db_and_tables'][0]} \n error: {error}''')
    conn.close()


# Create tables business_info, access_info, yellowpages_info,
# tripadvisor_info, foursquare_info
def create_tables(db):
    ...
    try:
        ...
        logger.error(f'''DATABASE QUERY FAILED
                     {queries['create_db_and_tables'][1]} \n error: {error}''')
    conn.close()
